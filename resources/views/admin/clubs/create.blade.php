    ...
    <div class="py-6 max-w-lg mx-auto px-4">
        <div class="bg-blue-900 shadow rounded-lg p-6">
            <form method="POST" action="{{ route('super.clubs.store') }}" class="space-y-4">@csrf
                ...
                <div><label class="block text-sm font-medium text-blue-200">Playtomic Club ID <span class="text-blue-500 font-normal">(optional)</span></label><input type="text" name="playtomic_tenant_id" value="{{ old('playtomic_tenant_id') }}" class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md placeholder-blue-500" placeholder="e.g. 2ab75436-9bb0-4e9c-9a6f-b12931a9ca4a"><p class="text-blue-500 text-xs mt-1">Found in your Playtomic club URL.</p>@error('playtomic_tenant_id')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror</div>
