<x-app-layout>
    ...
    <div class="py-6 max-w-lg mx-auto px-4">
        <div class="bg-blue-900 shadow rounded-lg p-6">
            <form method="POST" action="{{ route('admin.matches.doubles.store', [$club, $season]) }}" class="space-y-4">@csrf
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-blue-200">Pair 1</label><select name="pair1_id" required class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md"><option value="">Select</option>@foreach($pairs as $pair)<option value="{{ $pair->id }}" {{ old('pair1_id') == $pair->id ? 'selected' : '' }}>{{ $pair->displayName() }}</option>@endforeach</select>@error('pair1_id')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror</div>
                    <div><label class="block text-sm font-medium text-blue-200">Pair 2</label><select name="pair2_id" required class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md"><option value="">Select</option>@foreach($pairs as $pair)<option value="{{ $pair->id }}" {{ old('pair2_id') == $pair->id ? 'selected' : '' }}>{{ $pair->displayName() }}</option>@endforeach</select>@error('pair2_id')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror</div>
                </div>
