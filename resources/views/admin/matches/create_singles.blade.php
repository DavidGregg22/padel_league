<x-app-layout>
    ...
    <div class="py-6 max-w-lg mx-auto px-4">
        <div class="bg-blue-900 shadow rounded-lg p-6">
            <form method="POST" action="{{ route('admin.matches.singles.store', [$club, $season]) }}" class="space-y-4"
                  x-data="{ p1: '{{ old('player1_id', '') }}', p2: '{{ old('player2_id', '') }}' }">
                @csrf
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-blue-200">Player 1</label>
                        <select name="player1_id" required x-model="p1"
                                class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md">
                            <option value="">Select</option>
                            @foreach($players as $p)
                                <option value="{{ $p->id }}" :disabled="p2 == '{{ $p->id }}'">{{ $p->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="player2_id" required x-model="p2"
                                class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md">
                            <option value="">Select</option>
                            @foreach($players as $p)
                                <option value="{{ $p->id }}" :disabled="p1 == '{{ $p->id }}'">{{ $p->name }}</option>
                            @endforeach
                        </select>
