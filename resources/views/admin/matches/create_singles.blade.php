<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Add Singles Match — {{ $season->name }}</h2>
    </x-slot>
    ...
    <div class="py-8 max-w-lg mx-auto px-4">
        <div class="bg-white shadow rounded-lg p-6">
            <form method="POST" action="{{ route('admin.matches.singles.store', $season) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Score (Sets won by P1)</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Score (Sets won by P2)</label>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-md text-sm hover:bg-indigo-700">Save Match</button>
                </div>
            </form>
        </div>
    </div>
