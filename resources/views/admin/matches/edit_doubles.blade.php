<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Edit Doubles Match — {{ $season->name }}</h2>
    </x-slot>
    ...
    <div class="py-8 max-w-lg mx-auto px-4">
        <div class="bg-white shadow rounded-lg p-6">
            <form method="POST" action="{{ route('admin.matches.doubles.update', [$season, $match]) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="pair1_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach($pairs as $pair)
                                <option value="{{ $pair->id }}" {{ $match->pair1_id == $pair->id ? 'selected' : '' }}>
                                    {{ $pair->displayName() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="pair2_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach($pairs as $pair)
                                <option value="{{ $pair->id }}" {{ $match->pair2_id == $pair->id ? 'selected' : '' }}>
                                    {{ $pair->displayName() }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Sets won by Pair 1</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Sets won by Pair 2</label>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-md text-sm hover:bg-green-700">Save Changes</button>
                    </div>
                </div>
            </form>
        </div>
