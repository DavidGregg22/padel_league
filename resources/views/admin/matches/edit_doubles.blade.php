<x-app-layout>
    ...
    <div class="py-6 max-w-lg mx-auto px-4">
        <div class="bg-blue-900 shadow rounded-lg p-6">
            <form method="POST" action="{{ route('admin.matches.doubles.update', [$club, $season, $match]) }}" class="space-y-4">@csrf @method('PATCH')
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-blue-200">Pair 1</label><select name="pair1_id" required class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md">@foreach($pairs as $pair)<option value="{{ $pair->id }}" {{ $match->pair1_id == $pair->id ? 'selected' : '' }}>{{ $pair->displayName() }}</option>@endforeach</select></div>
                    <div><label class="block text-sm font-medium text-blue-200">Pair 2</label><select name="pair2_id" required class="mt-1 block w-full bg-blue-800 border-blue-700 text-white rounded-md">@foreach($pairs as $pair)<option value="{{ $pair->id }}" {{ $match->pair2_id == $pair->id ? 'selected' : '' }}>{{ $pair->displayName() }}</option>@endforeach</select></div>
                </div>
